...
const VerifyQR = () => {
  ...
  const handleVerify = async (t?: string) => {
    const tokenToVerify = t || token;
    if (!tokenToVerify.trim()) return;
    setLoading(true);
    try {
      const res = await adminAPI.verifyQR(tokenToVerify);
      setResult(res.data);
    } catch (err) {
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        <div style={styles.header}>
          ...
          <p style={styles.subtitle}>Permit status will appear below</p>
        </div>
